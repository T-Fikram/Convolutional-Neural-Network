...
if model is not None:
    ...
    if final_image is not None:
        ...
        with col1:
            st.image(final_image, caption='Input Gambar',
        ...
        with col2:
            ...
            if st.button("Analisis Emosi 🔍", key="btn_predict"):
                with st.spinner('Sedang memproses...'):
                    class_idx, conf, all_probs = predict_image(
                        final_image, model, device)
                    ...
                    probs_dict = {k: float(v)
                                  for k, v in zip(CLASS_NAMES, all_probs)}
